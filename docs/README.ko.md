...
- [ ] **공개 API**: 업로드, 페이지 관리, 외부 서비스 연동을 위한 문서화된 프로그래밍 인터페이스를 추가할 예정입니다.
- [ ] **MCP 서버**: AI 에이전트와 각종 도구 워크플로에서 mdto를 프로그래밍 방식으로 다룰 수 있도록 MCP 기반 통합 지점을 추가할 예정입니다.
- [ ] **마크다운 이미지 자동 업로드**: 마크다운 문서에 포함된 이미지를 mdto에 자동으로 업로드해서, 사용자가 이미지를 따로 외부에 호스팅하지 않아도 되게 할 예정입니다.
